...
3. **날짜가 바뀐 이벤트는 새 이벤트**: 같은 영화라도 날짜가 달라지면 다른 event_id가 생성되므로 재발송한다. 의도된 동작.
4. **새 이벤트 없으면 발송 안 함**: 빈 이메일 방지.
5. **새 극장 추가 방법**: `BaseCrawler`를 상속하여 `crawl() -> list[Event]`만 구현하면 됨.
...
- 크롤러 테스트는 실제 네트워크 요청 없이 `tests/fixtures/` HTML/JSON으로만 테스트
